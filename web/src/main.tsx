...
import './styles.css';
import { canEnterZone, canWalkTile, explore, findTilePath, getNearestPoi, interactWithPoi, getReachableZones, getZone, getZoneStatus, generateScenario, getTile, moveTile, zones } from './world';
...
function App() {
  ...
  <section className="panel"><div><span className="eyebrow">FACTION</span><h2>Choose your allegiance</h2></div><div className="actions">{factions.map(f=><button className={player.faction===f?'selected':''} key={f} onClick={()=>chooseFaction(f)}>{f}</button>)}</div><p>Current faction: <strong>{player.faction}</strong></p></section>
      <section className="panel"><div><span className="eyebrow">DYNAMIC SCENARIO</span><h2>{scenario.title}</h2><p>{scenario.description}</p><small>Threat level: {scenario.threat} · Scenario ID: {scenario.id}</small><div className="scenario-choices">{scenario.choices.map(choice=><button key={choice} type="button" onClick={()=>setWorldMessage('Scenario choice: '+choice)}>{choice}</button>)}</div></div></section>
