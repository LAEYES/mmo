...
function App() {
  ...
  useEffect(() => {
    ...
    if (path.length <= 1) {
      setAutoMove(false);
      setPathLength(0);
      const poi = getNearestPoi(currentZone, worldTile);\n      if (poi && poi.distance === 0) { const interaction=interactWithPoi(currentZone,poi.index); if(interaction){setPoiMessage('POI reached: '+interaction.name);setPoiAction(interaction.action); update(applyPoiReward(player,interaction.action)); setScenario(generateScenario(currentZone, player.level, player.explorationCount + (interaction.action === 'explore' ? 1 : 0))); setWorldEvent(generateWorldEvent(currentZone,player.worldThreat,player.worldResources,player.explorationCount));}}\n      setWorldMessage('Waypoint reached.');
      return;
    }
  ...
  <section className="panel"><div><span className="eyebrow">EMERGENT WORLD EVENT</span><h2>{worldEvent.title}</h2><p>{worldEvent.description}</p><small>{worldEvent.faction} · Intensity {worldEvent.intensity} · Effect {worldEvent.effect}</small></div></section>
