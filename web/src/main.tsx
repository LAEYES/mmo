...
<section className="panel"><div><span className="eyebrow">DYNAMIC SCENARIO</span><h2>{scenario.title}</h2><p>{scenario.description}</p><small>Threat level: {scenario.threat + player.worldThreat - 1} · Resources: {player.worldResources} · Scenario ID: {scenario.id}</small><div className="scenario-choices">{scenario.choices.map((choice,index)=><button key={choice} type="button" onClick={()=>{update(applyScenarioChoice(player,index));setWorldMessage('Scenario choice: '+choice);setScenario(generateScenario(currentZone,player.level,player.explorationCount,index+1));}}>{choice}</button>)}</div></div></section>
