...
function App() {
  ...
  <section className="panel"><div><span className="eyebrow">FACTION</span><h2>Choose your allegiance</h2></div><div className="actions">{factions.map(f=><button className={player.faction===f?'selected':''} key={f} onClick={()=>chooseFaction(f)}>{f}</button>)}</div><p>Current faction: <strong>{player.faction}</strong></p></section>
        <section className="panel"><div><span className="eyebrow">ENVIRONMENT · 2D / 3D READY</span><h2>{environmentLabels[environment.cycle]} · {weatherLabels[environment.weather]}</h2><div className="environment-grid"><article><strong>Cycle</strong><span>{environmentLabels[environment.cycle]}</span></article><article><strong>Météo</strong><span>{weatherLabels[environment.weather]}</span></article><article><strong>Atmosphère</strong><span>Niveau {environment.atmosphere}</span></article><article><strong>Menace</strong><span>Niveau {player.worldThreat}</span></article></div><p>État déterministe : le rendu, les VFX et les futures scènes 3D peuvent se synchroniser sur cette même couche d'environnement.</p></div></section>
  ...
  <section className="panel"><div><span className="eyebrow">EMERGENT WORLD EVENT</span><h2>{worldEvent.title}</h2><p>{worldEvent.description}</p><small>{worldEvent.faction} · Intensity {worldEvent.intensity} · Effect {worldEvent.effect}</small><div className="actions"><button type="button" onClick={()=>{const next=applyWorldEventState(player,worldEvent.effect,worldEvent.intensity,worldEvent.faction);update(next);setWorldEvent(generateWorldEvent(currentZone,next.worldThreat,next.worldResources,next.explorationCount));setWorldMessage('World event resolved: '+worldEvent.title);}}>Resolve event</button></div></div></section>
<section className="panel"><div><span className="eyebrow">DYNAMIC SCENARIO</span><h2>{scenario.title}</h2><p>{scenario.description}</p><small>Threat {getZoneDynamicModifiers(currentZone,player.worldThreat,player.worldResources,player.factionStates.find(f=>f.faction===player.faction)?.influence??100).threat} · Yield {getZoneDynamicModifiers(currentZone,player.worldThreat,player.worldResources,player.factionStates.find(f=>f.faction===player.faction)?.influence??100).resourceYield} · Encounter {getZoneDynamicModifiers(currentZone,player.worldThreat,player.worldResources,player.factionStates.find(f=>f.faction===player.faction)?.influence??100).encounterChance}%</small><div className="scenario-choices">{scenario.choices.map((choice,index)=><button key={choice} type="button" onClick={()=>{update(applyScenarioChoice(player,index));setWorldMessage('Scenario choice: '+choice);setScenario(generateScenario(currentZone,player.level,player.explorationCount,index+1));}}>{choice}</button>)}</div></div></section>
