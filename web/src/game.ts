...
export function createStarterPlayer(name = 'Arena Player'): PlayerState {
  return { name, level: 1, xp: 0, faction: 'Aegis', victories: 0, cards: [], zoneId: 'outpost', explorationCount: 0, lastDiscovery: '' };
}
...
export function loadPlayer(): PlayerState {
  try {
    ...
    return { ...createStarterPlayer(), ...parsed, cards: Array.isArray(parsed.cards) ? parsed.cards : [] };
  } catch {
